...
class TestZPyWallet(unittest.TestCase):
    ...
    def test_000_create_wallet(self):
        ...
        hdw = wallet.create_wallet(
            mnemonic="nature story debris circle decrease post "
            "gesture cute burger chef silent grief",
            network=BitcoinSegwitMainNet,
        )
        assert (
            hdw.dump_str_xkey(private=True)
    ...
    def test_001_bip32(self):
        ...
        hdw = HDWallet.from_master_seed(
            binascii.unhexlify("000102030405060708090a0b0c0d0e0f"),
            network=BitcoinSegwitMainNet,
        )
        assert (
            hdw.dump_str_xkey(private=False)
    ...
    def test_006_bip84(self):
        ...
        assert (
            hdw1.dump_str_xkey(private=True, segwit=True)
            == "zprvAdG4iTXWBoARxkkzNpNh8r6Qag3irQB8PzEMkAFeTRXxHpbF9z4QgEvBRmfvqWvGp42t42nvgGpNgYSJA9iefm1yYNZKEm7z6qUWCroSQnE"
        )
        assert (
            hdw1.dump_str_xkey(private=False, segwit=True)
        ...
        assert (
            hdw2.private_key.to_wif(compressed=True)
            == "KyZpNDKnfs94vbrwhJneDi77V6jF64PWPF8x5cdJb8ifgg2DUc9d"
        )
        assert (
            hdw2.public_key.to_hex(compressed=True)
            == "0330d54fd0dd420a6e5f8d3624f5f3482cae350f79d5f0753bf5beef9c2d91af3c"
        )
        assert (
            hdw2.public_key.bech32_address(compressed=True, witness_version=0)
        ...
        assert (
            hdw3.private_key.to_wif(compressed=True)
            == "Kxpf5b8p3qX56DKEe5NqWbNUP9MnqoRFzZwHRtsFqhzuvUJsYZCy"
        )
        assert (
            hdw3.public_key.to_hex(compressed=True)
            == "03e775fd51f0dfb8cd865d9ff1cca2a158cf651fe997fdc9fee9c1d3b5e995ea77"
        )
        assert (
            hdw3.public_key.bech32_address(compressed=True, witness_version=0)
        ...
        assert (
            hdw4.private_key.to_wif(compressed=True)
            == "KxuoxufJL5csa1Wieb2kp29VNdn92Us8CoaUG3aGtPtcF3AzeXvF"
        )
        assert (
            hdw4.public_key.to_hex(compressed=True)
            == "03025324888e429ab8e3dbaf1f7802648b9cd01e9b418485c5fa4c1b9b5700e1a6"
        )
        assert (
            hdw4.public_key.bech32_address(compressed=True, witness_version=0)
    ...
    def test_007_brainwallet(self):
        ...
        w = HDWallet.from_brainwallet(
            "crazy horse battery staple", network=BitcoinSegwitMainNet
        )
        assert (
            w.private_key.to_wif(compressed=True)
            == "KzJp5B7mDpZ7kMHv67GowQRys9W9Hbaa5Rzj4PCoiyXfTk1fGAvH"
        )
    ...
    def test_008_bip38(self):
        ...
        assert (
            bip38key.private_key("TestingOneTwoThree", compressed=False).to_wif(
                compressed=False
            )
